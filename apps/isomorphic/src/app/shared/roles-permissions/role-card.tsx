...
export default function RoleCard({
  ...
  const handleDelete = async () => {
    ...
    const confirmed = window.confirm(
      `¿Estás seguro de eliminar el rol "${name}"?\nEsta acción no se puede deshacer y puede producir errores si existen usuarios con este rol.`
    );
    if (!confirmed) return;
    ...
    try {
      ...
      if (res.ok) {
        toast.success(`Rol "${name}" eliminado con éxito.`);
        onDelete?.();
      } else {
        ...
        try {
          ...
          errorMessage = 'No se pudo leer el mensaje de error del servidor.';
        }
  ...
  return (
    <div className={cn('rounded-lg border border-muted p-6', className)}>
        ...
        <Dropdown className={className} placement="bottom-end">
          ...
          <Dropdown.Menu className="!z-0">
            <Dropdown.Item
              ...
              className="gap-2 text-xs font-medium text-red-600 hover:text-red-700 sm:text-sm"
            >
              Eliminar Rol
            </Dropdown.Item>
          </Dropdown.Menu>
      ...
      <ModalButton
        ...
        view={<EditRole id={id} currentName={name} />}
