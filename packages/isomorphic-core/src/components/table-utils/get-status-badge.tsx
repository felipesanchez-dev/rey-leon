...
export function getStatusBadge(status?: string) {
  const safeStatus = (status || 'default').toLowerCase() as StatusTypes;

  if (safeStatus in allStatus) {
    return (
      <Flex align="center" gap="2" className="w-auto">
        <Badge renderAsDot className={allStatus[safeStatus][1]} />
        <Text
          className={cn("font-medium capitalize", allStatus[safeStatus][0])}
        >
          {replaceUnderscoreDash(safeStatus)}
        </Text>
      </Flex>
    );
  }
